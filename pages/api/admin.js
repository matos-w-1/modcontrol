...
export default function Admin() {
  ...
  async function loadAll() {
    loadMods(); loadRequests(); loadSwaps(); loadAttendance()
  }
  ...
  const shiftCell = (mod) => mod.role==='admin'
    ? <span style={{color:D.hint,fontSize:12}}>—</span>
    : badge(...(shiftBadge[mod.shift]||['#333','#aaa']),mod.shift)

  const pending = requests.filter(r => r.status === 'pending')
  const pendingSwaps = swaps.filter(s => s.status === 'waiting_admin')
  const todayStr = new Date().toISOString().slice(0,10)
  const todayLogs = attendanceLogs.filter(a => a.created_at?.slice(0,10) === todayStr)
    ...
    <div style={{minHeight:'100vh',background:D.bg,fontFamily:'system-ui,sans-serif',display:'flex',color:D.text}}>
      <div style={{width:220,flexShrink:0,background:D.sidebar,borderRight:`0.5px solid ${D.border}`,display:'flex',flexDirection:'column',minHeight:'100vh'}}>
        ...
        <div style={{padding:'12px 10px',flex:1}}>
          {[['dashboard','📊 Dashboard'],['approvals',`✅ Approvals${pending.length>0?' ('+pending.length+')':''}`],['swaps',`🔄 Swaps${pendingSwaps.length>0?' ('+pendingSwaps.length+')':''}`],['mods','👥 Moderators'],['attendance','🕐 Attendance']].map(([id,label]) => (
        ...
        <div style={{flex:1,overflowY:'auto',padding:20}}>
          ...
          {page==='mods' && editMod && <>
            <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:16}}>
              ...
              {editMod.role!=='admin' && <button style={{...btn('#7f1d1d',false),padding:'6px 12px',fontSize:12,marginLeft:'auto'}} onClick={()=>deleteMod(editMod)}>🗑 Remove</button>}
            </div>
            <div style={card}>
              ...
              <form onSubmit={saveMod}>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:12}}>
                  ...
                  {editMod.role!=='admin' && <div><label style={{fontSize:12,fontWeight:500,color:D.muted,display:'block',marginBottom:5}}>Shift</label>
                ...
                <tbody>
                  {attendanceLogs.map(a=>(
                    <tr key={a.id}>
                      ...
                      <td style={{padding:'8px 8px',borderBottom:`0.5px solid ${D.border}`}}>{a.profiles?.name && mods.find(m=>m.id===a.user_id)?.role==='admin' ? <span style={{color:D.hint}}>—</span> : badge(...(shiftBadge[a.profiles?.shift]||['#333','#aaa']),a.profiles?.shift||'—')}</td>
